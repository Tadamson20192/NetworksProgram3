...
class NetworkPacket:
    ...
    MF_length = 1
    offset_length = 2
    header_length = dst_addr_S_length + MF_length + offset_length
    
    ##@param dst_addr: address of the destination host
    # @param data_S: packet payload
    def __init__(self, dst_addr, data_S, MF = 0, offset = 0):
    ...
    def to_byte_Segm(self):
        byte_S = str(self.dst_addr).zfill(self.dst_addr_S_length)
        byte_S += str(self.MF).zfill(self.MF_length)
        byte_S += str(self.offset).zfill(self.offset_length)
        byte_S += self.data_S
        return byte_S
		
    def isSegment(self, byte_S):
...
class Host:
    ...
    def udt_send(self, dst_addr, data_S, MTU):
        if len(data_S) > MTU:
            split1 = data_S[0: int(len(data_S)/2)]
            split2 = data_S[int(len(data_S) / 2) : ]
            p1 = NetworkPacket(dst_addr, split1)
            self.out_intf_L[0].put(p1.to_byte_S())
            print('%s: sending packet "%s" on the out interface with mtu=%d' % (self, p1, self.out_intf_L[0].mtu))
            p2 = NetworkPacket(dst_addr, split2)
    ...
    def forward(self):
        for i in range(len(self.in_intf_L)):
            ...
            try:
                ...
                if pkt_S is not None:
                    packets = NetworkPacket.from_byte_S(pkt_S, 30) #parse a packet out
                    # HERE you will need to implement a lookup into the 
                    # forwarding table to find the appropriate outgoing interface
                    # for now we assume the outgoing interface is also i
                    for p in packets:
                        self.out_intf_L[i].put(p.to_byte_Segm(), True)
